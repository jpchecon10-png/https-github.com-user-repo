    ...
    <div className="min-h-screen bg-white tracking-[-0.02em]" style={{ fontFamily: "'Inter', sans-serif" }}>
      <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between p-4 sm:p-5">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
          <button type="button" className="px-4 py-1.5 rounded-full text-sm font-medium text-white bg-white/20 transition-colors">
            Course
          </button>
          {NAV_LINKS.map((label) => (
            <button
        ...
        <div className="flex items-center gap-3">
          <button type="button" className="hidden md:block bg-white text-gray-900 text-sm font-semibold px-6 py-2.5 rounded-full hover:bg-gray-100">
            Sign Up
          </button>
          <button type="button" className="md:hidden text-white" aria-label="Open menu">
            <Menu size={24} />
          </button>
        </div>
      </nav>
